/** Session metric formulas — keep in sync with extension/shared/session-metrics.js */

export const AE1_MIN_POSTS = 30;
export const AE1_TARGET_RATE = 0.7;
...
export function applyDecision(metrics, action) {
...
export function applyOversight(metrics, resolution) {
  return applyDecision(metrics, resolution);
}

export function computeFilterRate(metrics) {
  if (!metrics?.total) return null;
  return (metrics.hide + metrics.block) / metrics.total;
}

export function computeQuietRate(metrics) {
  if (!metrics?.total) return null;
...
export function computeSnr(metrics) {
  if (!metrics?.total) return null;
  return metrics.show / metrics.total;
}

export function ae1ProbeStatus(metrics) {
